...
class ProductService {
    async getAll({ limit = 20, offset = 0, search, category, character, sortBy } = {}) {
        ...
        if (sortBy === 'price_asc') {
            ...
            order = [[Product.sequelize.literal('(SELECT COALESCE(SUM(quantity), 0) FROM order_items WHERE order_items.product_id = Product.product_id)'), 'DESC']];
        } else if (sortBy === 'popular') {
            order = [[Product.sequelize.literal('(SELECT COUNT(DISTINCT order_id) FROM order_items WHERE order_items.product_id = Product.product_id)'), 'DESC']];
        }
